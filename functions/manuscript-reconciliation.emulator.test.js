...
async function getFixture(idToken){
  return fetch(FIXTURE_URL,{headers:idToken?{authorization:`Bearer ${idToken}`}:{}});
}
...
test("isolated emulator verifies the Firebase Auth to /api/me roles-array contract",async t=>{
  ...
  try{
    await t.test("signed-out requests are denied",async()=>{
      ...
      const fixtureResponse=await getFixture();
      assert.equal(fixtureResponse.status,401);
      assert.deepEqual(await fixtureResponse.json(),{error:"auth"});
    });
    const identities={};
    ...
    await t.test("synthetic non-team roles do not satisfy reconciliation authorization",async()=>{
      ...
      const fixtureResponse=await getFixture(identities.viewer.idToken);
      assert.equal(fixtureResponse.status,403);
      assert.deepEqual(await fixtureResponse.json(),{error:"forbidden",requiredRoles:["team","admin"]});
    });
