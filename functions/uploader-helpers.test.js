...
test("public image verification falls back to detected bytes for generic object metadata", () => {
  ...
  assert.equal(verifiedImageContentType("application/octet-stream", ""), "application/octet-stream"); }); test("explicitly confirmed no-poem QI uses the source filename stem and remains approval-gated", () => { const cells = Array(33).fill(""); cells[0] = "2025"; cells[4] = "Book Specific Quote Graphic - AEO 1.png"; cells[6] = "drive-no-poem"; cells[11] = "an everyday occurrence"; cells[12] = "Fall 2025"; cells[15] = "Hailey Tran"; cells[16] = "AEO"; cells[20] = "ready_for_poetry_please_ingestion"; cells[26] = "user_confirmed_no_poem_qi_uploaded"; cells[28] = "author_qi_book_confirmed_no_poem"; const selected = selectQiLibraryYearRows([[], cells], { year: "2025", approvedSourceRows: new Set([2]) }); assert.equal(selected.remainingCount, 1); assert.equal(selected.rows[0].title, "Book Specific Quote Graphic - AEO 1"); cells[28] = ""; const blocked = selectQiLibraryYearRows([[], cells], { year: "2025", approvedSourceRows: new Set([2]) }); assert.equal(blocked.remainingCount, 0); assert.equal(blocked.reviewRows[0].error, "missing_qi_library_content_id");
});
